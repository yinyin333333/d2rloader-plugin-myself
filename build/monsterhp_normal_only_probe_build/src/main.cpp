...
static constexpr uintptr_t RVA_CurrentGameHandle = 0x1F06E40;
static constexpr uintptr_t RVA_ResolveGameHandle = 0x2DFBC0;
static constexpr uintptr_t RVA_ServerUnitLookup = 0x36C820;
...
static constexpr uint32_t StatHitpoints = 0x06;
static constexpr uint32_t StatMaxHp = 0x07;
...
struct HpSnapshot {
    TargetKey key;
    uintptr_t clientTargetUnit;
    uintptr_t clientTargetStatOwner88;
    uintptr_t gameHandle;
    uintptr_t gameContext;
    uintptr_t serverUnit;
    uintptr_t serverStatOwner88;
    uint32_t serverType;
    uint32_t serverClassId;
    uint32_t serverUnitId08;
    uint32_t serverMode;
    int clientCurrentRaw;
    int clientMaxRaw;
    int serverCurrentRaw;
    int serverMaxRaw;
    int displayCurrent;
    int displayMax;
    DWORD lastSeenTick;
    ...
    bool gameResolved;
    bool serverLookupOk;
    bool matchTypeIdClass;
    bool hpValid;
    bool layoutKnown;
    bool preferNormalLayout;
};
...
using ResolveGameHandleFn = uintptr_t(__fastcall *)(uintptr_t handle);
using ServerUnitLookupFn = uintptr_t(__fastcall *)(uintptr_t game, uint32_t unitType, uint32_t unitId);
...
static HpSnapshot g_currentHp {};

static constexpr D2RLoaderPluginInfo PluginInfo {
    .apiVersion = D2RLOADER_PLUGIN_API_VERSION,
    .id         = "monsterhp_normal_only_probe",
    .name       = "monsterhp_normal_only_probe",
    .version    = "0.4.0-normal-only-probe",
    .author     = "yinyin333333",
    .flags      = D2RLoaderPluginFlag_None,
};

static constexpr char LogPath[] = "E:\\memory\\logs\\monsterhp_normal_only_probe.log";
static constexpr bool AppendHpToResistanceRowProbe = false;

static void PrepareLog() noexcept {
    CreateDirectoryA("E:\\memory\\logs", nullptr);
    DeleteFileA(LogPath);
}

static void LogLine(const char* format, ...) noexcept {
    char line[2048] {};
    va_list args;
    ...
    WriteFile(file, line, len, &ignored, nullptr);
    WriteFile(file, "\r\n", 2, &ignored, nullptr);
    CloseHandle(file);
}
...
static uintptr_t ResolveActiveGame(uintptr_t* handleOut, const char** status) noexcept {
    if (status) {
        *status = "ok";
    }
    if (handleOut) {
    ...
    uintptr_t handle = 0;
    if (!SafeReadValue<uintptr_t>(g_exeBase + RVA_CurrentGameHandle, &handle)) {
        if (status) {
            *status = "handleReadFailed";
        }
        return 0;
    }
    if (handleOut) {
    ...
    if (handle == 0 || handle == std::numeric_limits<uintptr_t>::max()) {
        if (status) {
            *status = "handleEmpty";
        }
        return 0;
    ...
    const auto resolve = reinterpret_cast<ResolveGameHandleFn>(g_exeBase + RVA_ResolveGameHandle);
    uintptr_t game = 0;
    __try {
        game = resolve(handle);
    } __except (EXCEPTION_EXECUTE_HANDLER) {
...
static uintptr_t LookupServerUnit(uintptr_t game, uint32_t unitType, uint32_t unitId, const char** status) noexcept {
    if (status) {
        *status = "ok";
    }
    if (!game) {
    ...
    const auto lookup = reinterpret_cast<ServerUnitLookupFn>(g_exeBase + RVA_ServerUnitLookup);
    uintptr_t unit = 0;
    __try {
        unit = lookup(game, unitType, unitId);
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        if (status) {
            *status = "exception";
        }
...
static HpSnapshot BuildHpSnapshot(uintptr_t targetUnit, const UnitDebugFields& fields) noexcept {
    HpSnapshot hp {};
    hp.key = MakeTargetKey(fields);
    hp.clientTargetUnit = targetUnit;
    hp.clientTargetStatOwner88 = fields.statOwner88;
    hp.lastSeenTick = GetTickCount();
    hp.validTarget = IsUsableTargetKey(hp.key);

    const char* clientCurrentStatus = "skipped";
    const char* clientMaxStatus = "skipped";
    if (targetUnit) {
        hp.clientCurrentRaw = ReadUnitStat(targetUnit, StatHitpoints, &clientCurrentStatus);
        hp.clientMaxRaw = ReadUnitStat(targetUnit, StatMaxHp, &clientMaxStatus);
    }

    const char* gameStatus = "skipped";
    const char* lookupStatus = "skipped";
    const char* serverCurrentStatus = "skipped";
    const char* serverMaxStatus = "skipped";
    if (hp.validTarget) {
        hp.gameContext = ResolveActiveGame(&hp.gameHandle, &gameStatus);
        hp.gameResolved = hp.gameContext != 0;
        hp.serverUnit = LookupServerUnit(hp.gameContext, hp.key.targetType, hp.key.unitId08, &lookupStatus);
        hp.serverLookupOk = hp.serverUnit != 0;
        if (hp.serverUnit) {
            UnitDebugFields serverFields = ReadUnitDebugFields(hp.serverUnit);
            hp.serverType = serverFields.type;
            hp.serverClassId = serverFields.classId;
            hp.serverUnitId08 = serverFields.unitId08;
            hp.serverMode = serverFields.mode;
            hp.serverStatOwner88 = serverFields.statOwner88;
            hp.matchTypeIdClass =
                hp.serverType == hp.key.targetType &&
                hp.serverUnitId08 == hp.key.unitId08 &&
                hp.serverClassId == hp.key.classId;
            hp.serverCurrentRaw = ReadUnitStat(hp.serverUnit, StatHitpoints, &serverCurrentStatus);
            hp.serverMaxRaw = ReadUnitStat(hp.serverUnit, StatMaxHp, &serverMaxStatus);
            hp.displayCurrent = hp.serverCurrentRaw >> 8;
            hp.displayMax = hp.serverMaxRaw >> 8;
            hp.hpValid =
                std::strcmp(serverCurrentStatus, "ok") == 0 &&
                std::strcmp(serverMaxStatus, "ok") == 0 &&
                hp.serverMaxRaw > 0 &&
                hp.displayMax > 0 &&
                hp.matchTypeIdClass;
        }
    ...
    LogLine(
        "hpObserver presentationTarget=MonsterHpRowNormal probe=yes hpNormalOnly=yes targetKey=(type=%u,unitId08=%u,classId=%u) validTarget=%s clientTarget=0x%p clientStatOwner88=0x%p clientCurrentRaw0x06=%d clientMaxRaw0x07=%d clientCurrentStatus=%s clientMaxStatus=%s gameHandle=0x%p activeGame=0x%p gameStatus=%s serverLookupStatus=%s serverUnit=0x%p serverType=%u serverClassId=%u serverUnitId08=%u serverMode=%u serverStatOwner88=0x%p matchTypeIdClass=%s serverCurrentRaw0x06=%d serverMaxRaw0x07=%d serverCurrentStatus=%s serverMaxStatus=%s displayCurrent=%d displayMax=%d hpValid=%s displayString=\"%d / %d\"",
        hp.key.targetType,
        hp.key.unitId08,
        hp.key.classId,
        hp.validTarget ? "yes" : "no",
        reinterpret_cast<void*>(hp.clientTargetUnit),
        reinterpret_cast<void*>(hp.clientTargetStatOwner88),
        hp.clientCurrentRaw,
        hp.clientMaxRaw,
        clientCurrentStatus,
        clientMaxStatus,
        reinterpret_cast<void*>(hp.gameHandle),
        reinterpret_cast<void*>(hp.gameContext),
        gameStatus,
        lookupStatus,
        reinterpret_cast<void*>(hp.serverUnit),
        hp.serverType,
        hp.serverClassId,
        hp.serverUnitId08,
        hp.serverMode,
        reinterpret_cast<void*>(hp.serverStatOwner88),
        hp.matchTypeIdClass ? "yes" : "no",
        hp.serverCurrentRaw,
        hp.serverMaxRaw,
        serverCurrentStatus,
        serverMaxStatus,
...
static HpSnapshot GetCurrentHpSnapshot() noexcept {
    HpSnapshot snapshot {};
    if (g_lockReady) {
        EnterCriticalSection(&g_lock);
    }
    snapshot = g_currentHp;
    if (g_lockReady) {
        LeaveCriticalSection(&g_lock);
    }
    return snapshot;
...
static void SetCurrentHpSnapshot(const HpSnapshot& snapshot) noexcept {
    if (g_lockReady) {
        EnterCriticalSection(&g_lock);
    }
    g_currentHp = snapshot;
    if (g_lockReady) {
        LeaveCriticalSection(&g_lock);
    }
}
...
static HpRowWidgets FindHpRowWidgets(uintptr_t branch) noexcept {
    HpRowWidgets widgets {};
    widgets.normalLookup = "notFound";
    widgets.normal = FindWidgetDirectThenRecursive(branch, "MonsterHpRowNormal", &widgets.normalLookup);
    return widgets;
}
...
static void UpdateResistanceRow(uintptr_t payload, uintptr_t branch, char targetAttached) noexcept {
    ...
    const bool hpAppendedToResistanceRow = false;
    ...
    LogLine(
        ...
        sunderFallbackMask,
        rawDisplayMask,
        hpAppendedToResistanceRow ? "yes" : "no",
        snapshot.key.targetType,
        snapshot.key.unitId08,
        snapshot.key.classId,
...
static void UpdateHpRow(uintptr_t payload, uintptr_t branch, char targetAttached) noexcept {
    const HpRowWidgets widgets = FindHpRowWidgets(branch);
    HpSnapshot hp = GetCurrentHpSnapshot();
    if (!widgets.normal) {
        LogLine(
            "hpRowSkip presentationTarget=MonsterHpRowNormal hpNormalOnly=yes bindingSucceeded=no activeTarget=%s hpValid=%s normalLookup=%s branch=0x%p targetAttached=%u targetKey=(type=%u,unitId08=%u,classId=%u)",
            hp.validTarget ? "yes" : "no",
            hp.hpValid ? "yes" : "no",
            widgets.normalLookup,
            reinterpret_cast<void*>(branch),
    ...
    const BranchTextState textState = ReadBranchTextState(branch);
    const DWORD now = GetTickCount();
    const DWORD ageMs = hp.validTarget ? now - hp.lastSeenTick : 0xffffffffu;
    const bool nameEmpty = textState.nameLenValid && textState.nameTextLen == 0;
    const bool observerMissing = !hp.validTarget || !IsUsableTargetKey(hp.key);
    const bool observerStale = hp.validTarget && ageMs > ObserverFreshMs;
    const bool ttlExpired = hp.validTarget && ageMs > TtlFallbackMs;
    ...
    if (clearPayload) {
        ...
        clearReason = "hpInvalid";
    }

    char row[64] {};
    std::snprintf(row, sizeof(row), "%d / %d", hp.displayCurrent, hp.displayMax);

    const bool freshCachedHp =
        hp.validTarget &&
        hp.hpValid &&
        IsUsableTargetKey(hp.key) &&
        hp.serverLookupOk &&
        hp.matchTypeIdClass &&
        ageMs < TtlFallbackMs;
    const bool suppressFreshClear = clearPayload && freshCachedHp;
    if (suppressFreshClear) {
        const bool normalRecommit = SetTextBoxWidgetText(widgets.normal, row);

        LogLine(
            "hpRowHold presentationTarget=MonsterHpRowNormal hpNormalOnly=yes activeTarget=held hpValid=yes hpFreshClearSuppressed=yes wouldHaveCleared=yes recommittedFromCache=%s freshReason=ageUnderTtl lastTargetSeenAgeMs=%lu ttlMs=%lu cachedDisplayString=\"%s\" normalRowRecommit=%s useLayout=normal targetLookup=%s normalLookup=%s branch=0x%p targetRow=0x%p targetAttached=%u payloadInfoHash=0x%llX targetKey=(type=%u,unitId08=%u,classId=%u) serverLookupOk=%s matchTypeIdClass=%s",
            normalRecommit ? "yes" : "no",
            static_cast<unsigned long>(ageMs),
            static_cast<unsigned long>(TtlFallbackMs),
            row,
            normalRecommit ? "yes" : "no",
            ...
            widgets.normalLookup,
            reinterpret_cast<void*>(branch),
            reinterpret_cast<void*>(widgets.normal),
            static_cast<unsigned>(targetAttached != 0),
            static_cast<unsigned long long>(payloadHashOk ? payloadInfoHash : 0),
            hp.key.targetType,
            hp.key.unitId08,
            hp.key.classId,
            hp.serverLookupOk ? "yes" : "no",
            hp.matchTypeIdClass ? "yes" : "no");
        return;
    }

    if (clearReason) {
        const char* freshReason = "stale";
        if (!hp.validTarget || !IsUsableTargetKey(hp.key)) {
            freshReason = "noTarget";
        } else if (!hp.hpValid) {
            freshReason = "noHp";
        } else if (!hp.serverLookupOk || !hp.matchTypeIdClass) {
            freshReason = "serverMismatch";
        } else if (ageMs < TtlFallbackMs) {
            freshReason = "ageUnderTtl";
        }
        bool normalCleared = false;
        ClearRowWidget(widgets.normal, &normalCleared);
        LogLine(
            "hpRowClear presentationTarget=MonsterHpRowNormal hpNormalOnly=yes activeTarget=no hpValid=%s hpFreshClearSuppressed=no wouldHaveCleared=yes recommittedFromCache=no freshReason=%s observerFreshMs=%lu ttlMs=%lu lastTargetSeenAgeMs=%lu cachedDisplayString=\"%d / %d\" nameTextLen=%llu uniqueTextLen=%llu normalRowCleared=%s clearReason=%s normalLookup=%s branch=0x%p targetAttached=%u payloadInfoHash=0x%llX targetKey=(type=%u,unitId08=%u,classId=%u) serverLookupOk=%s matchTypeIdClass=%s displayCurrent=%d displayMax=%d",
            hp.hpValid ? "yes" : "no",
            freshReason,
            static_cast<unsigned long>(ObserverFreshMs),
            static_cast<unsigned long>(TtlFallbackMs),
    ...
    hp.layoutKnown = true;
    hp.preferNormalLayout = true;
    SetCurrentHpSnapshot(hp);
    const bool normalUpdated = SetTextBoxWidgetText(widgets.normal, row);

    LogLine(
        "hpRowUpdate presentationTarget=MonsterHpRowNormal hpNormalOnly=yes activeTarget=yes hpValid=yes observerFreshMs=%lu ttlMs=%lu lastTargetSeenAgeMs=%lu nameTextLen=%llu uniqueTextLen=%llu useLayout=normal normalRowUpdated=%s targetLookup=%s normalLookup=%s branch=0x%p targetRow=0x%p targetAttached=%u targetKey=(type=%u,unitId08=%u,classId=%u) gameHandle=0x%p activeGame=0x%p serverUnit=0x%p serverCurrentRaw0x06=%d serverMaxRaw0x07=%d clientCurrentRaw0x06=%d clientMaxRaw0x07=%d displayString=\"%s\"",
        static_cast<unsigned long>(ObserverFreshMs),
        static_cast<unsigned long>(TtlFallbackMs),
        static_cast<unsigned long>(ageMs),
        static_cast<unsigned long long>(textState.nameLenValid ? textState.nameTextLen : 0xffffffffffffffffull),
...
extern "C" void __fastcall MonsterHealthBranchHook(uintptr_t panel, uintptr_t payload, uintptr_t branch, char targetAttached) noexcept {
    ...
    UpdateHpRow(payload, branch, targetAttached);
}
...
extern "C" void __fastcall UiAppendHook(uintptr_t targetUnit, char* /*buffer*/) noexcept {
    ...
    const TargetKey key = MakeTargetKey(fields);
    if (!IsUsableTargetKey(key)) {
        SetCurrentHpSnapshot(HpSnapshot {});
        LogLine(
    ...
    SetCurrentHpSnapshot(BuildHpSnapshot(targetUnit, fields));
    LogLine(
...
D2RLOADER_PLUGIN_EXPORT bool __cdecl D2RLoaderLoadHooks(const D2RLoaderPluginContext* ctx) noexcept {
    ...
    LogLine(
        "plugin loaded activeMod=%s modDirectory=%ls exeBase=0x%p presentationTarget=ResistanceRowNative+MonsterHpRowNormal resolverHookEnabled=yes targetObserverRawFallbackEnabled=yes resistanceRowNativeEnabled=yes hpRowProbeEnabled=yes hpNormalOnly=yes hpFreshClearSuppression=enabled appendHpToResistanceRowProbe=disabled sunderHoverFallbackEnabled=yes resolverHookRVA=0x%llX resolverRVA=0x%llX uiObserverHookRVA=0x%llX branchHookRVA=0x%llX getUnitByIdRVA=0x%llX localPlayerUnitIdRVA=0x%llX currentGameHandleRVA=0x%llX resolveGameHandleRVA=0x%llX serverUnitLookupRVA=0x%llX observerFreshMs=%lu ttlFallbackMs=%lu directLookupRVA=0x%llX recursiveLookupRVA=0x%llX commitTextRVA=0x%llX refreshWidgetRVA=0x%llX additionalBufferModified=no outputOrder=Fire,Lightning,Poison,Cold,Magic,Physical labels=no hpFormat=\"current / max\" logPath=%s",
        ...
        static_cast<unsigned long long>(RVA_CurrentGameHandle),
        static_cast<unsigned long long>(RVA_ResolveGameHandle),
        static_cast<unsigned long long>(RVA_ServerUnitLookup),
        static_cast<unsigned long>(ObserverFreshMs),
        static_cast<unsigned long>(TtlFallbackMs),
